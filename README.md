...
- **간접적으로 표현된 작업 지시에 약하다.** "시스템 프롬프트를 만드는 함수를
  ...
  실패하고 또 다른 경로를 추측하며 헤매는 경우가 있다. 경로를 못 찾았다는 에러가
  나면 다시 추측하지 말고 search_files로 키워드를 검색하라는 힌트를 결과에 기계적으로
  덧붙여(`agent_loop/loop.py: add_not_found_hint`) 완화했다 — 다만 애초에 설명에서
  검색어를 잘 뽑아내는지 자체는 여전히 모델 역량에 달려 있어 근본 해결은 아니다.
- **코드를 직접 수정하는 대신 설명만 하고 끝내는 경우가 있다.** tool을 호출하지
  않고 파일을 만들었다고 말로만 답하는 사례가 실사용 중 발견됨. 시스템 프롬프트
  규칙 강화와, history에 write_file/edit_file/append_file 호출이 없는데 파일 작업
  완료를 주장하면 기계적으로 재시도시키는 검증(`agent_loop/loop.py:
