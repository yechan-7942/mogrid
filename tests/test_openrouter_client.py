...
class CallOpenRouterTests(unittest.TestCase):
    ...
    @patch("router.openrouter_client.requests.post")
    def test_success_returns_content(self, mock_post):
        mock_post.return_value = FakeResponse(
            200, {"choices": [{"message": {"content": "안녕하세요"}}]}
        )
        self.assertEqual(call_openrouter("안녕"), "안녕하세요")

    @patch("router.openrouter_client.requests.post")
    def test_null_content_raises_openrouter_error(self, mock_post):
    ...
    @patch("router.openrouter_client.requests.post")
    def test_empty_string_content_raises_openrouter_error(self, mock_post):
        mock_post.return_value = FakeResponse(
    ...
    @patch("router.openrouter_client.requests.post")
    def test_timeout_raises_openrouter_error(self, mock_post):
        mock_post.side_effect = requests.exceptions.Timeout()
        with self.assertRaises(OpenRouterError):
            call_openrouter("안녕")

    @patch("router.openrouter_client.requests.post")
    def test_401_raises_openrouter_error(self, mock_post):
        mock_post.return_value = FakeResponse(401, text="unauthorized")
        with self.assertRaises(OpenRouterError):
            call_openrouter("안녕")

    @patch("router.openrouter_client.requests.post")
    def test_429_raises_openrouter_error(self, mock_post):
        mock_post.return_value = FakeResponse(429, text="rate limited")
        with self.assertRaises(OpenRouterError):
            call_openrouter("안녕")

    @patch("router.openrouter_client.requests.post")
    def test_malformed_body_raises_openrouter_error(self, mock_post):
